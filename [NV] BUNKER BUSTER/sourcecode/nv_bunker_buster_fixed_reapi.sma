...
// For nade Mode Plugin you using if any.. 
...
new g_selected,g_has_Bunker,g_zoom,Float:pOrigin[33][3],m_iTrail,m_iSmoke,m_iExp,m_iFire,m_iBlackS,g_maxplayer,g_iPlayer_Mode[33],g_iCvar[10],g_iEntExist[500];
...
public Native_Is_Bunker(Ent)
{
	if(pev_valid(Ent))
	{
		if(get_entvar(Ent,var_impulse) == WEAPON_KEY)
			return true;
	}
	return false;
}
...
{
	//set_task(5.0,"Say_Sequence_Number",id,_,_,"b");
	Remove_Bunker(id);
}
...
public Drop_Bunker(id)
{
	if(is_user_alive(id))
	{
		if(get_user_weapon(id) == CSW_BUNKER && Get_BitVar(g_has_Bunker, id))
		{
			...
			rg_remove_item(id,weapon_bunker);
			
			#if defined KNIFE_ITEM
			rg_giveitem(id,weapon_bunker);	
			#endif
...
public fw_NM_nade_property_set(entity, NadeType:nade_type)
{
	new id = get_entvar(entity,var_owner);
	if(Get_BitVar(g_has_Bunker,id) || get_entvar(entity,var_impulse) == WEAPON_KEY)
		return PLUGIN_HANDLED;
...
public Give_Bunker(id)
{
	if(is_user_alive(id))
	{
		if(!Get_BitVar(g_has_Bunker,id))
		{
			...
			set_entvar(id,var_viewmodel,MODELS[M_V_MODEL]);
			set_entvar(id,var_weaponmodel,MODELS[M_P_MODEL]);
		...
		else
		{
			...
			static Ent; Ent = rg_find_weapon_bpack_by_name(id, weapon_bunker);
			if(pev_valid(Ent))
			{
				rg_set_user_bpammo(id, WeaponIdType:get_weaponid(weapon_bunker), rg_get_user_bpammo(id,WeaponIdType:get_weaponid(weapon_bunker))+1);
			}
		}
		#endif
	}
...
{
	new ent;
	while((ent = rg_find_ent_by_class(ent,C_Plane)) != 0)
	{
		if(pev_valid(ent))
		{
			remove_task(ent);
	...
	while((ent = rg_find_ent_by_class(ent,C_Target)) != 0)
	{
		if(pev_valid(ent))
		{
			remove_task(ent);
...
public Remove_Entities_By_Id(id)
{
	...
	while((ent = rg_find_ent_by_class(ent,C_Missile))!= 0)
	{
		if(pev_valid(ent))
		{
			if(get_entvar(ent,var_iuser1) == id)
		...
		if(Get_BitVar(g_has_Bunker,id) && cs_get_user_weapon(id) == CSW_BUNKER)
		{
			...
			if(iButton & IN_ATTACK)
			{
				if(Get_BitVar(g_zoom,id))
				{
					if(!Get_BitVar(g_selected,id))
					{
						...
						pOrigin[id][0] = hOrigin[0];
						pOrigin[id][1] = hOrigin[1];
						pOrigin[id][2] = hOrigin[2];
					...
					if(get_entvar(id,var_weaponanim) != 0) set_weapon_anim(id,0,1);
				}
			...
			if(OldButton & IN_ATTACK && !(iButton & IN_ATTACK))
			{
				...
				if(Get_BitVar(g_selected,id) && Get_BitVar(g_zoom,id))
				{
					...
					sOrigin[0] = pOrigin[id][0];
					sOrigin[1] = pOrigin[id][1];
					sOrigin[2] = pOrigin[id][2];
					// If The Starting and Ending Point are Same or Maybe Too Near 
					// Than Throw Only One BOMB... at Aim Position....
					
					
					if(get_distance_f(sOrigin,eOrigin) < 100.0)
					{
						Create_Missile(id,sOrigin);
						Create_Target(id,eOrigin);
					}
					else
					{
						Create_Target(id,sOrigin);
						Create_Target(id,eOrigin);
						Create_Plane(id,sOrigin,eOrigin);
						
					}
					
					if(get_entvar(id,var_weaponanim) != 0) set_weapon_anim(id,0,0);
			...
			if(OldButton & IN_ATTACK2 && !(iButton & IN_ATTACK2))
			{
				...
				if(!Get_BitVar(g_zoom,id))
				{
					...
					set_entvar(id,var_viewmodel,MODELS[M_V_MODEL_SIGHT]);
				}
				else
				{	
					UnSet_BitVar(g_zoom,id);
...
public fw_SetModel(entity, model[])
{
	...
	static id;
	id = get_entvar(entity, var_owner);
...
public Fw_Touch(iEnt, iTouch)
{
	if(is_valid_ent(iEnt))
	{
		...
		// This is to remove Missile Automatically
		
		set_entvar(iEnt,var_iuser2,0);
		...
		{	
			...
			set_entvar(iEnt, var_origin, Origin);
			//set_entvar(iEnt, var_model,"");
...
public fw_Item_AddToPlayer_Post(Ent, id)
{
	...
	if(Get_BitVar(g_has_Bunker, id))
	{
		set_entvar(Ent,var_impulse,WEAPON_KEY);
		Wpnlist(id,1);
	}
	else
	{
...
public Create_Plane(id,Float:sOrigin[3],Float:eOrigin[3])
{
	...
	if(i_Ent)
	{
		...
		set_entvar(i_Ent, var_classname, C_Plane);
		set_entvar(i_Ent, var_solid, SOLID_NOT);
		set_entvar(i_Ent, var_movetype, MOVETYPE_NOCLIP);
		
		sOrigin[2] = GetMaxHeight(i_Ent,sOrigin) + 50;
		
		set_entvar(i_Ent, var_origin,sOrigin);
		set_entvar(i_Ent, var_iuser1,id);
		
		set_entvar(i_Ent, var_vuser1,sOrigin);
		set_entvar(i_Ent, var_vuser2,eOrigin);
		...
		set_entvar(i_Ent, var_fuser1,get_gametime() + 20.0);
		
		//set_task(20.0,"Kill_Remove",i_Ent);
		set_task(4.0,"Task_Fly",i_Ent);
		
		set_entvar(i_Ent, var_nextthink,get_gametime() + 4.0);
...
public Create_Fake_World(id) 
{
	...
	new i_Ent = rg_create_entity("info_target");
	if(i_Ent)
	{
		set_entvar(i_Ent, var_classname, C_Bunker);
		entity_set_model(i_Ent,MODELS[M_W_MODEL]);
		set_entvar(i_Ent, var_solid, 1);
		set_entvar(i_Ent, var_movetype, 6);
		set_entvar(i_Ent, var_origin,origin);
		engfunc(EngFunc_SetSize, i_Ent,Float:{-2.0,-2.0,-2.0},Float:{5.0,5.0,5.0});
...
public Create_Target(id,Float:sOrigin[3])
{
	...
	set_entvar(i_Ent, var_classname, C_Target);
	set_entvar(i_Ent, var_solid, SOLID_NOT);
	set_entvar(i_Ent, var_movetype, MOVETYPE_TOSS);
	set_entvar(i_Ent, var_origin,sOrigin);
	set_entvar(i_Ent, var_iuser1,id);
	set_entvar(i_Ent, var_light_level, 180);
	set_entvar(i_Ent, var_rendermode, kRenderTransAdd);
	set_entvar(i_Ent, var_renderamt, 255.0);
...
public Create_Missile(id,Float:sOrigin[3])
{
	...
	set_entvar(i_Ent, var_classname, C_Missile);
	set_entvar(i_Ent, var_solid, SOLID_BBOX);
	set_entvar(i_Ent, var_movetype, MOVETYPE_TOSS);
	//engfunc(EngFunc_SetSize, i_Ent, Float:{-34.0, -34.0, -94.0}, Float:{34.0, 34.0, 95.0});
	engfunc(EngFunc_SetSize,i_Ent,Float:{ -15.0, -15.0, 0.0 }, Float:{ 15.0, 15.0, 12.0 });
	set_entvar(i_Ent, var_iuser1,id);
	set_entvar(i_Ent, var_iuser2,1);
	...
	vAngle[0] -= 90.0;
	set_entvar(i_Ent, var_angles, vAngle);
...
public Create_Fire(taskid)
{
	...
	if(pev_valid(ent))
	{
		new iOwner = get_entvar(ent,var_iuser1);
		new Float:Origin[3];
...
public Think_Missile(ent)
{
	if(pev_valid(ent)) 
	{
		...
		if(get_entvar(ent,var_iuser2))
		{
			Kill_Remove(ent);
		}
		
		if(pev_valid(ent)) set_entvar(ent, var_nextthink,get_gametime() + 0.1);
	}
}
...
public Think_Plane(ent)
{
	if(pev_valid(ent))
	{
		...
		new id = get_entvar(ent,var_iuser1);
		
		if(is_user_alive(id))
		{
			get_entvar(ent,var_origin,Origin);
			get_entvar(ent,var_vuser1,sOrigin);
			get_entvar(ent,var_vuser2,eOrigin);
		...
		get_entvar(ent, var_fuser1, fTimeRemove);
		if (get_gametime() >= fTimeRemove)
		{
			Kill_Remove(ent);
		}
		
		if(pev_valid(ent)) set_entvar(ent, var_nextthink,get_gametime() + 1.0);
	}
}
...
public Aim_To_Target(iEnt, Float:vTargetOrigin[3])
{
	...
	get_entvar(iEnt, var_origin, Vec);
	...
	set_entvar(iEnt, var_v_angle, Angles);
	set_entvar(iEnt, var_angles, Angles);
}
...
{ 
	...
	return ( ( pcCurrent == CONTENTS_SKY )) ? fOrigin[ 2 ] - 100.0 : fOrigin[ 2 ];
}
...
stock ham_strip_weapon(id,weapon[])
{
	...
	new wEnt;
	while((wEnt = engfunc(EngFunc_FindEntityByString,wEnt,"classname",weapon)) && pev(wEnt,var_owner) != id) {}
	if(!wEnt) return 0;
	...
	set_entvar(id,var_weapons,pev(id,var_weapons) & ~(1<<wId));
	return 1;
}
...
{ 
	...
	get_entvar(id, var_v_angle, fvAngle); 
	get_entvar(id, var_view_ofs, fvOffset); 
...
if(is_valid_ent(ent))
{
	...
	set_entvar(ent, var_animtime, halflife_time());
